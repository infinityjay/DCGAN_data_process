...
% Path = 'D:\DCGAN\IGS_TEC\20160101_0330\';     % 设置数据存放的文件夹路径
...
% for k = 83 : Length_Names
%     dir = strcat(Path, FileNames{k});
    dir = 'D:\DCGAN\IGS_TEC\20160101_0330\gps_tec2hr_igs_20160101_v01.cdf'
%     info = cdfinfo(dir);
%     vars = info.Variables;
    ...
    y=cell2mat(lat);
    x=cell2mat(lon);
    for i = 1:1 %两小时平均，一天12张图
%      figure(i);
         fig(i) = figure('Visible', 'on'); %不显示图片
         z= tecIGS{i,1};
         img = pcolor(x,y,z);
        ...
        %保存图片
%          dir2 = ['D:\python_projects\data_result_test\',num2str(k),'_',num2str(2*i),'.jpg'];
%          saveas(gcf,dir2);
